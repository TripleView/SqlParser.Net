...
true || false && true，这段逻辑运算的顺序是什么
 在逻辑运算中，运算符的优先级决定了运算的顺序。通常，逻辑运算符的优先级如下：
逻辑非（NOT，通常用 "!" 表示）
逻辑与（AND，通常用 "&&" 表示）
逻辑或（OR，通常用 "||" 表示）
根据这个优先级，逻辑与运算符 "&&" 的优先级高于逻辑或运算符 "||"。因此，在表达式 "true || false && true" 中，逻辑与运算会先进行。
具体的运算顺序如下：
计算 "false && true"，结果是 false。
然后计算 "true || false"，结果是 true。
因此，整个表达式 "true || false && true" 的结果是 true。
