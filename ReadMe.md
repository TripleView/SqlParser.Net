...
DistinctOn子句，是PostgreSQL特有的语法，用于：按指定列分组，每组只保留一条记录。它通常与 ORDER BY 配合，用来获取每组中“最新的一条”“最早的一条”或“优先级最高的一条”数据。在本例子中值为SqlDistinctOnExpression，代表这是一个DistinctOn表达式，它包含了

1. DistinctOn依据的项列表，即Items字段，item字段里的值是一个表达式，在本例子中值为1个SqlIdentifierExpression和1个SqlFunctionCallExpression
...
CastAs子句，用于表达式转换，常见用法为CAST(表达式 AS 数据类型)。在本例子中值为SqlCastAsExpression，代表这是一个CastAs子句，它包含了

1. 待转换的表达式，即Body字段，在本例子中值为1个SqlIdentifierExpression
2. 目标类型，即TargetType字段，在本例子中值为1个SqlIdentifierExpression
3. FunctionType转换方式，包括Function，ColonColon（即pgsql的::），TypeString（即pgsql的类型-表达式）
